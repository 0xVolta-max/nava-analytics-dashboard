  ...
  return (
    <div className="bg-white/10 backdrop-blur-xl border border-white/15 rounded-2xl p-6 flex flex-col h-full">
      <div className="flex justify-between items-center mb-4">
        <h3 className="font-semibold text-white/90 text-lg">Generation Activity</h3>
        <Badge variant="outline" className="border-white/20 text-white/80">Last 30 Days</Badge>
      </div>
      
      <div className="flex-grow flex flex-col justify-center">
        <div className="flex gap-4">
          <div className="flex flex-col justify-between text-sm text-white/50 py-1">
            <span>Mon</span>
            <span>Wed</span>
            <span>Fri</span>
          </div>
          <div className="grid grid-flow-col grid-rows-7 grid-cols-5 gap-2.5 w-full">
            {data.map((value, index) => (
              <div
                key={index}
                className={`aspect-square rounded-md ${getColor(value)}`}
              />
            ))}
          </div>
        </div>
      ...
      <div className="flex justify-end items-center gap-2 mt-5 text-sm text-white/50">
        <span>Less</span>
        <div className="flex gap-1.5">
            {Array.from({ length: 5 }).map((_, i) => (
                <div key={i} className={`w-3.5 h-3.5 rounded-sm ${getColor(i)}`} />
            ))}
        </div>
        <span>More</span>
      </div>
